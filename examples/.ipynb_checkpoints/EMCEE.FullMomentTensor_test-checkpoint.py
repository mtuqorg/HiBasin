...
if __name__=='__main__':
    ...
    if comm.rank==0:
        ...
        with multiprocessing.Pool() as pool:
            log_prob_fn = log_prob_noiseamp_timeshift_Fullmt
        ...
        mean_sol = get_solution(sampler, warm_up_steps=int(0.5*nsteps), thin=10,source_type='full')
        ...
        flat_samples_out.tofile(chain_dir +'EMCEE_sw_noise_fmt_model.bin')
        log_prob_samples = sampler.get_log_prob(discard=0, thin=1, flat=False)
        log_prob_samples.tofile(chain_dir +'EMCEE_sw_noise_fmt_log_prob.bin')
    ...
    if comm.rank==0:
        ...
        plot_data_greens1(event_id+'FMT_waveforms_sw_noise.png',
            data_sw, greens_sw, process_sw, 
            misfit_sw, stations, origin, best_mt, lune_dict)


        plot_beachball(event_id+'FMT_beachball_sw_noise.png',
            best_mt, stations, origin)
        
        plot_waveform_fit(best_mt.as_vector(), data_sw_array, greens_sw_array, stations, noise, tau, event_id+'FMT_waveformfit_sw_noise.png')
