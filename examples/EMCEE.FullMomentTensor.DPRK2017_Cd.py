...
if __name__=='__main__':
    ...
    if comm.rank==0:
        ...
        ##shift the GFs with the pre-solved time shifts        
        tau_shift = np.array([-1.87,-2.98, -1.59, -2.83, 0.71, -1.14, -1.07,-0.06, -3.01,-10.2, -2.57,6.93])
        greens_sw = shift_greens(greens_sw, tau_shift)
        ...
        noise_estimator = covariace_matrix(origin, data_noise, npts_acf_lag, noise_length=3000, noise_model='empirical')
        ...
        noise_estimator.plot_auto_corr_func()
        noise_estimator.plot_data_covariance_matrix('covariance_matrix.png')
    ...
    if comm.rank==0:
        ...
        solver = MCMC_FullMij(misfit_sw, data_sw, greens_sw,
                          noise_std_sw, cov_inv=cov_inv, log_cov_det=log_cov_det,
                          max_noise_parameter=400, M00=1.0e15, noise_type='correlated')
    ...
    if comm.rank==0:
        ...
        plot_data_greens1(event_id+'_Mij_waveforms_sw_d%skm_noise_covm.png' % evdp_in_km,
            data_sw, greens_sw, process_sw, 
            misfit_sw, stations, origin, best_mt, lune_dict)

        plot_beachball(event_id+'_Mij_beachball_sw_d%skm_noise_covm.png' % evdp_in_km,
            best_mt, stations, origin)
        
        plot_waveform_fit(best_mt.as_vector(), solver.obs, solver.greens, stations, noise_sol, tau_sol, event_id+'_Waveformfit_mean_covm.jpg', delta=1, evdp_in_km=evdp_in_km)

        #
        # Plot the posterior distribution
        posterior_distribution_mij(flat_samples_fname=solver.chain_fname,log_prob_fname=solver.logprob_fname, mt_degree=solver.ne, thin=2,ratio=0.5, figure_fname=event_id+"_Posterior_source_parameter_covm.jpg")
        posterior_distribution_noise(flat_samples_fname=solver.chain_fname, mt_degree=solver.ne, thin=10, ratio=0.5,stations=stations, figure_fname=event_id+'_Posterior_data_noise_covm.jpg')
        posterior_distribution_timeshift(solver, thin=10, ratio=0.5,stations=stations, figure_fname=event_id+'_Posterior_timeshift_covm.jpg')
        print(noise_sol)
        print(tau_sol)
        print('\nFinished\n')
