...
if __name__=='__main__':
    ...
    if comm.rank==0:
        ...
        with multiprocessing.Pool() as pool:
            log_prob_fn = log_prob_noiseamp_timeshift_DC
        ...
        mean_sol = get_solution(sampler, warm_up_steps=int(0.5*nsteps), thin=10, source_type='dc')
        ...
        chain_dir = './'
        noise_std_sw.tofile(chain_dir + 'noise_std_sw_sigma.bin')
        flat_samples_out = sampler.get_chain(discard=0, thin=1, flat=False)
        flat_samples_out.tofile(chain_dir +'EMCEE_sw_noise_dc_model.bin')
        log_prob_samples = sampler.get_log_prob(discard=0, thin=1, flat=False)
        log_prob_samples.tofile(chain_dir +'EMCEE_sw_noise_dc_log_prob.bin')
    ...
    if comm.rank==0:
        ...
        plot_data_greens1(event_id+'DCMT_waveforms_sw_noise.png',
            data_sw, greens_sw, process_sw, 
            misfit_sw, stations, origin, best_mt, lune_dict)


        plot_beachball(event_id+'DCMT_beachball_sw_noise.png',
            best_mt, stations, origin)

        plot_waveform_fit(best_mt.as_vector(), data_sw_array, greens_sw_array, stations, noise, tau, event_id+'DC_waveformfit_sw_noise_new.png')
        ...
        plot_data_greens2(event_id+'DC_waveforms_sw_noise_inversion.png',
            data_bw, data_sw, greens_bw, greens_sw, process_bw, process_sw, 
            misfit_bw, misfit_sw, stations, origin, best_mt, lune_dict)


        plot_beachball(event_id+'DC_beachball_sw_noise_inversion.png',
            best_mt, stations, origin)
