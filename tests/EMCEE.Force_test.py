...
#         ne=3 #for SF
        ...
        mean_sol = get_solution(sampler, warm_up_steps=int(0.5*nsteps), thin=10, source_type='force')
        ...
        chain_dir = './'
        noise_std_sw.tofile(chain_dir + 'EMCEE_noise_std_sw_sf_sigma.bin')
        flat_samples_out = sampler.get_chain(discard=0, thin=5, flat=False)
        flat_samples_out.tofile(chain_dir +'EMCEE_sw_noise_sf_model.bin')
        log_prob_samples = sampler.get_log_prob(discard=0, thin=5, flat=False)
        log_prob_samples.tofile(chain_dir +'EMCEE_sw_noise_sf_log_prob.bin')
    ...
    if comm.rank==0:
        ...
        plot_data_greens1(event_id+'SF_waveforms_sw_noise.png',
        ...
        plot_data_greens2(event_id+'SF_waveforms_sw_noise_inversion.png',
            data_bw, data_sw, greens_bw, greens_sw, process_bw, process_sw, 
            misfit_bw, misfit_sw, stations, origin, best_force, force_dict)


        plot_beachball(event_id+'SF_beachball_sw_noise_inversion.png',
            best_force, stations, origin)
