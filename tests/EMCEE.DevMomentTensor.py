...
from utils.misfit_preparation import misfit_preparation
...
#     ns,nc,nt = data_sw_array.shape
#     fig, ax = plt.subplots(1,nc,sharex=True,sharey=True, figsize=(7.5,5))
#     for s in range(ns):
#         for c in range(nc):
#             scale = 0.5 / np.max(np.abs(data_sw_array[s]))
#             ax[c].plot(data_sw_array[s,c]*scale+s)
#     plt.show()
