...
#         greens.convolve(wavelet)
    ...
    noise_std_sw = noise_std_sw*3 #5 for 1km#5 iso, 8-clvd. 3 for 30km
    #ISO source
...
#     mt = ned2rtp(np.array([2.760, -1.661, -1.099, 0.707, 1.048, 0.634])*3.0e16)
    ...
    if comm.rank==0:
        ...
        ns,nc,ne,nt = greens_sw_array.shape
        np.random.seed(1000)
        ##number of unknowns: 3-comps of one station share the same time shift
        ndim = ne + ns + ns
        nwalker = 128
        nsteps = 5000
        MAXVAL = 3600
        ...
        with multiprocessing.Pool() as pool:
            log_prob_fn = log_prob_noisecov_onetimeshift_Fullmt
            ## Initializa the sample
            sampler = emcee.EnsembleSampler(nwalker, ndim, log_prob_fn, args=[data_sw_array, greens_sw_array, noise_std_sw,cov_inv, log_cov_det], pool=pool)
            ## Running MCMC
        ...
        mean_sol = get_solution(sampler, warm_up_steps=int(0.5*nsteps), thin=100, source_type='full')
        ...
        flat_samples_out.tofile(chain_dir +'EMCEE_sw_noise_tt2015_syn_d%skm_model_cd.bin' % evdp_in_km)
        log_prob_samples = sampler.get_log_prob(discard=0, thin=5, flat=False)
        log_prob_samples.tofile(chain_dir +'EMCEE_sw_noise_tt2015_syn_d%skm_log_prob_cd.bin' % evdp_in_km)
        ...
        tau = np.mean(flat_samples[:,ne+ns:], axis=0) / 1800#360
        print("noise: ", noise)
        print("tau: ", tau)
    ...
    if comm.rank==0:
        ...
        tau_recon = np.zeros(2*ns)
        for s in range(ns):
            tau_recon[2*s] = tau[s]
            tau_recon[2*s+1] = tau[s] 
        greens_sw = shift_greens(greens_sw, tau_recon)
        plot_data_greens1(event_id+'TT2015_waveforms_sw_syn_d%skm_noise_cd.png' % evdp_in_km,
            data_sw, greens_sw, process_sw, 
            misfit_sw, stations, origin, best_mt, lune_dict)


        plot_beachball(event_id+'TT2015_beachball_sw_syn_d%skm_noise_cd.png' % evdp_in_km,
            best_mt, stations, origin)
        
        plot_waveform_fit(best_mt.as_vector(), data_sw_array, greens_sw_array, stations, noise, tau_recon, event_id+'TT2015_waveformfit_sw_syn_d%skm_noise_cd.png' % evdp_in_km, evdp_in_km)
